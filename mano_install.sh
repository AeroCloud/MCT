...
COIN_TGZ='https://github.com/MasternodeFoundation/MANOcoin/releases/download/v1.1.0/MANO-bin-linux64.tar.gz'
...
purgeOldInstallation() {
    ...
    #kill wallet daemon
    sudo killall manod > /dev/null 2>&1
    #remove old ufw port allow
    sudo ufw delete allow 5982/tcp > /dev/null 2>&1
    #remove old files
    if [ -d "~/ge" ]; then
        sudo rm -rf ~/ge > /dev/null 2>&1
    fi
    #remove binaries and GIN utilities
    cd /usr/local/bin && sudo rm mano-cli mano-tx manod > /dev/null 2>&1 && cd
...
function install_sentinel() {
  ...
  echo "dash_conf=$CONFIGFOLDER/mano.conf" >> $CONFIGFOLDER/sentinel/sentinel.conf
}
...
function download_node() {
  ...
  cp MANO-bin/$COIN_DAEMON MANO-bin/$COIN_CLI $COIN_PATH
  cd ~ >/dev/null 2>&1
...
function important_information() {
 ...
 echo -e "${GREEN}mano-cli masternode status${NC}"
 echo -e "${GREEN}mano-cli getinfo.${NC}"
 echo -e "${BLUE}================================================================================================================================${NC}"
