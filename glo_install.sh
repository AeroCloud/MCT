...
COIN_TGZ='https://github.com/glorium-official/glorium-core/releases/download/1.1.5/glorium-1.1.5-x86_64-linux-gnu.tar.gz'
COIN_ZIP=$(echo $COIN_TGZ | awk -F'/' '{print $NF}')
SENTINEL_REPO='https://github.com/AeroCloud/sentinel'
COIN_NAME='GLO'
COIN_PORT=15777
RPC_PORT=15767
...
purgeOldInstallation() {
    ...
    #kill wallet daemon
    sudo killall gloriumd > /dev/null 2>&1
    #remove old ufw port allow
    sudo ufw delete allow 15777/tcp > /dev/null 2>&1
    #remove old files
    if [ -d "~/.glorium" ]; then
        sudo rm -rf ~/.glorium > /dev/null 2>&1
    fi
    #remove binaries and MCT utilities
    cd /usr/local/bin && sudo rm glorium-cli glorium-tx gloriumd > /dev/null 2>&1 && cd
...
function download_node() {
  ...
  chmod +x bin/$COIN_DAEMON bin/$COIN_CLI
...
function important_information() {
 ...
 echo -e "${GREEN}glorium-cli masternode status${NC}"
 echo -e "${GREEN}glorium-cli getinfo.${NC}"
 echo -e "${BLUE}================================================================================================================================${NC}"
